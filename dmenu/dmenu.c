...
drawmenu(void)
{
    ...
    if (using_vi_mode && text[0] != '\0') {
        ...
        drw_text(drw, x + curpos, 0, drw_fontset_getwidth(drw, vi_char) - lrpad, bh, 0, vi_char, 0);
	...
	for (item = items; item && item->text; item++) {
		if (text_len) {
			...
			for (i = 0; i < itext_len && (c = item->text[i]); i++) {
				/* fuzzy match pattern */
				if (tolower(text[pidx]) == tolower(c)) {
					if (sidx == -1)
						sidx = i;
					pidx++;
					if (pidx == text_len) {
...
usage(void)
{
	die("usage: dmenu [-bCfv] [-vi] [-l lines] [-p prompt] [-fn font] [-m monitor]\n"
	    "             [-nb color] [-nf color] [-sb color] [-sf color] [-w windowid]\n"
        "             [-c center] [-h height] [-mw menu_width]");
}
...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
		...
		if (!strcmp(argv[i], "-v")) {      /* prints version information */
			...
			fast = 1;
		else if (!strcmp(argv[i], "-c")) {   /* apperats at the cneter of the screen */
            center = 1;
		...
		/* these options take one argument */
			...
			embed = argv[++i];
		else if (!strcmp(argv[i], "-bw"))   /* embedding window id */
			border_width = atoi(argv[++i]);
		else if (!strcmp(argv[i], "-mw"))   /* embedding window id */
			width = atoi(argv[++i]);
		else
			usage();
